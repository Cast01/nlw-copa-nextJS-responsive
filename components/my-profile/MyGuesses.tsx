...
export function MyGuesses(props: MyGuessesPropsType) {
    return (
        <Link 
            href={`http://localhost:3000/room/${props.room.id}`} 
            ...
            <div className="p-4 w-full bg-[#202024] flex flex-col gap-5">
                <div className="w-full h-[50%]">
                    <div className="text-2xl font-black truncate">Sala do{"(a)"} {props.room.title}</div>
                    <div className="text-[#C4C4CC] text-sm">Criado por {props.room.owner.name}</div>
                </div>
                <div className="w-full h-[50%] flex items-center justify-center">
                    <ul className="flex relative left-4">
                        ...
                        {
                            props.room.Participant.length > 4 ? (
